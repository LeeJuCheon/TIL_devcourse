...
def convolution():
    ...
    X = np.arange(batch*in_c*in_h*in_w,dtype=np.float32).reshape([batch,in_c,in_h,in_w])
...
def shallow_network():
    ...
    L1_w = (X[1].shape[3] - W1.shape[2] + 2* padding) // stride + 1
    ...
    for e in range(num_epoch):
        ...
        for i in range(len(X)):
            ...
            #print("L1")
            #print(L1)
            L1_act = np.array(sigmoid(L1), dtype=np.float32)            #print("L1 act")
            #print(L1_act)
